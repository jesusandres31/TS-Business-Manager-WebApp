...
import { fetchTotalSoldToClients } from '../../../../../../../../features/statistics/statisticsSlice';
// interface
...
interface TotalSoldToClientsFilterProps {
  setTotalSoldToClients: React.Dispatch<
    React.SetStateAction<ITotalSoldToClients[]>
  >;
}

export default function TotalSoldToClientsFilter({
  setTotalSoldToClients,
}: TotalSoldToClientsFilterProps) {
  ...
  const handleFetchTotalSoldToClients = (formatedDate: IDate) => {
    dispatch(fetchTotalSoldToClients(formatedDate))
      .then(unwrapResult)
      .then((payload) => {
        setTotalSoldToClients(payload);
      })
      .catch((err) => {
        errorSnackbar();
        throw err;
  ...
  const handleFilter = () => {
    ...
    handleFetchTotalSoldToClients(formatedDate);
  };
