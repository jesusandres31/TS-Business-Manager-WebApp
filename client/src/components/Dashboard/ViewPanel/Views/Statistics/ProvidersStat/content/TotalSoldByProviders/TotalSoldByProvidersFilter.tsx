...
import { fetchTotalSoldByProviders } from '../../../../../../../../features/statistics/statisticsSlice';
// interface
...
interface TotalSoldByProvidersFilterProps {
  setTotalSoldByProviders: React.Dispatch<
    React.SetStateAction<ITotalSoldByProviders[]>
  >;
}

export default function TotalSoldByProvidersFilter({
  setTotalSoldByProviders,
}: TotalSoldByProvidersFilterProps) {
  ...
  const handleFetchTotalSoldByProviders = (formatedDate: IDate) => {
    dispatch(fetchTotalSoldByProviders(formatedDate))
      .then(unwrapResult)
      .then((payload) => {
        setTotalSoldByProviders(payload);
      })
      .catch((err) => {
        errorSnackbar();
        throw err;
  ...
  const handleFilter = () => {
    ...
    handleFetchTotalSoldByProviders(formatedDate);
  };
